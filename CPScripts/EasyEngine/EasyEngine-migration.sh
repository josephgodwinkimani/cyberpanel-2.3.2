...
ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/cyberpanel_migration_key "$sudoer wget -q -O /root/header.sh https://raw.githubusercontent.com/josephgodwinkimani/cyberpanel-2.3.2/stable/CPScripts/EasyEngine/header.sh ; $sudoer bash /root/header.sh ${domains[$i]}"
fi
}
...
ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/cyberpanel_migration_key "$sudoer wget -q -O /root/key.sh https://raw.githubusercontent.com/josephgodwinkimani/cyberpanel-2.3.2/stable/CPScripts/EasyEngine/key.sh ; $sudoer bash /root/key.sh disable"
...
#check the input , if it's a file , consider it as key.
  ssh -o StrictHostKeyChecking=no $user_name@$server_ip -p$server_port -i $password "$sudoer wget -q -O /root/key.sh https://raw.githubusercontent.com/josephgodwinkimani/cyberpanel-2.3.2/stable/CPScripts/EasyEngine/key.sh ; $sudoer bash /root/key.sh enable"
  if [[ $? == "0" ]] ; then
  ...
  sshpass -p "${password}" ssh -o StrictHostKeyChecking=no $user_name@$server_ip -p$server_port "$sudoer wget -q -O /root/key.sh https://raw.githubusercontent.com/josephgodwinkimani/cyberpanel-2.3.2/stable/CPScripts/EasyEngine/key.sh ; $sudoer bash /root/key.sh enable"
  if [[ $? == "0" ]] ; then
...
ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/cyberpanel_migration_key "$sudoer wget -q -O /root/ext.sh https://raw.githubusercontent.com/josephgodwinkimani/cyberpanel-2.3.2/stable/CPScripts/EasyEngine/ext.sh ; $sudoer bash /root/ext.sh"
